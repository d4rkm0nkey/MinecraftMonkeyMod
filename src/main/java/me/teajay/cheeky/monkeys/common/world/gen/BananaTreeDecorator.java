...
public class BananaTreeDecorator extends TreeDecorator {
   public static final Codec<BananaTreeDecorator> CODEC = Codec.floatRange(0.0F, 1.0F).fieldOf("probability")
         .xmap(BananaTreeDecorator::new, (treeDecorator) -> {
            ...
            while (var6.hasNext()) {
               ...
               if (random.nextFloat() <= 0.25F) {
                  ...
                  if (Feature.isAir(world, blockPos)) {
                     BlockState blockState = (BlockState) ((BlockState) CheekyMonkeys.BANANA_BLOCK.getDefaultState()
                           .with(BananaBlock.AGE, random.nextInt(3))).with(BananaBlock.FACING, direction);
